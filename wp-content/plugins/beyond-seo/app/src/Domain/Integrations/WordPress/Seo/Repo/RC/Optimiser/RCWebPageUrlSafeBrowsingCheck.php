...
/**
 * Class RCWebPageUrlSafeBrowsingCheck
 */
#[RCLoad(
...
class RCWebPageUrlSafeBrowsingCheck extends ValueObject
{
    use RCTrait;
    use RCSecurityTrait;
    use RcLoggerTrait;
    ...
    public function getLoadEndpoint(): ?string
    {
        ...
        $url = 'https://' . $prefix . '.rankingcoach.com/app/api/client/integrations/wordpress/seo/optimiser/webpage/isSafeBrowsing?debug=1&noCache=true';
    ...
    public function handleLoadResponse(mixed &$callResponseData = null): void
    {
        ...
        /** @var SafeBrowsingCheckOperation $parent */
        $parent = $this->getParent();
        if ($callResponseData && isset($callResponseData->analyse)) {
            $parent->safeBrowsingChecker->analyse = (array)$callResponseData->analyse ?? [];
        }
    }
}
