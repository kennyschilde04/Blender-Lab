...
class RCAnalyzeSemanticDepth extends ValueObject
{
    use RCTrait;
    use RCSecurityTrait;
    ...
    /**
     * Constructor initializing semantic depth structure.
     */
    ...
    /**
     * Get the endpoint for loading the semantic depth analysis
     *
     * @return string|null
     */
    public function getLoadEndpoint(): string|null
    {
        ...
        $url = 'https://' . $prefix . '.rankingcoach.com/app/api/client/integrations/wordpress/seo/optimiser/content/semanticDepth?debug=1&noCache=true';
    ...
    /**
     * Handle the API response and map data to the parent
     *
    ...
    public function handleLoadResponse(mixed &$callResponseData = null): void
    {
        ...
        if ($callResponseData && isset($callResponseData->analyse)) {
            $parent->semanticAnalysis->analyse = (array)$callResponseData->analyse ?? [];
        }
    }
}
