...
trait RCTrait
{
    ...
    /** @var string private caching for cacheKey (we want to avoid to call multiple times uniqueKey during loading operations */
     ...
     * @param Entity|ValueObject $entity
     * @return static
     * @throws InternalErrorException
     * @throws ReflectionException
     */
    public function fromEntity(Entity|ValueObject &$entity): static
    {
        $propertyNamesToSkip = [
            'children' => true,
            'parent' => true,
        ...
        foreach ($entity->getProperties() as $property) {
            ...
            if ($propertyName == 'elements' && $this instanceof ObjectSet) {
                ...
                foreach ($entity->elements as $element) {
                    $rcClassName = $element::getRepoClass(LazyLoadRepo::RC);
                    if ($rcClassName) {
                        /** @var RCTrait $rcInstance */
                        $rcInstance = new $rcClassName();
                        $rcInstance->fromEntity($element);
                        $rcInstance->setParent($this);
                        $this->add($rcInstance);
                    } else {
            ...
            if (!($type && !$typeIsBuiltin && $typeName)) {
                ...
                $rcClassName = $entity->getRepoClassForProperty(LazyLoadRepo::ARGUS, $propertyName);
                if (!$rcClassName && is_a($type->getName(),DefaultObject::class,true)) {
                    /** @var DefaultObject $typeClassName */
                    $typeClassName = $type->getName();
                    $rcClassName = $typeClassName::getRepoClass(LazyLoadRepo::RC);
                }
                //$propertyClass = $rcClassName = str_replace('\\Entities\\', '\\Repo\\Argus\\', $property->getType());
                if ($rcClassName && class_exists($rcClassName)) {
                    $rcInstance = new $rcClassName();
                    $propertyInstnace = $entity->$propertyName;
                    $propertyParent = $propertyInstnace->getParent();
                    if ($propertyParent) {
                        $rcInstance->setParent($propertyParent);
                    }
                    $rcInstance->fromEntity($entity->$propertyName);
                    $this->$propertyName = $rcInstance;
                    // if property is child of entity, we add $rcInstance also als our child
    ...
    public function toEntity(
        array $callPath = [],
        EntitySet|Entity|ValueObject &$entityInstance = null
    ): EntitySet|Entity|ValueObject|null {
