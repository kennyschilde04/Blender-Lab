...
class RCCache
{
    ...
    private static $defaultCachePrefix = 'RcCache_';
    ...
    public static function getMulti($keys): ?array
    {
        ...
        $rcCacheKeys = [];
        $rcCacheKeyToCacheKeyAllocation = [];
        ...
        foreach ($keys as $cacheKey) {
            $rcCacheKey = self::getCacheKey($cacheKey);
            $rcCacheKeyToCacheKeyAllocation[$rcCacheKey] = $cacheKey;
            $rcCacheKeys[] = $rcCacheKey;
        }
        $cachedRedisMulti = $cacheRedis->get(...$rcCacheKeys);
        if ($cachedRedisMulti && !is_array($cachedRedisMulti)) {
            // only one result returned
            $cachedRedisMulti = [$rcCacheKeys[0] => $cachedRedisMulti];
        }
        ...
        foreach ($cachedRedisMulti as $rcCacheKey => $cachedResult) {
            if (!$cachedResult) {
                continue;
            }
            $cacheKey = $rcCacheKeyToCacheKeyAllocation[$rcCacheKey];
    ...
    public static function get($cacheKey, $cacheLevel = self::CACHELEVEL_MEMORY_AND_DB): ?RCCacheItem
    {
        $cacheItem = new RCCacheItem();
