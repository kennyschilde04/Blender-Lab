...
namespace App\Symfony\ErrorHandlers;
...
class AppErrorHandler extends ErrorHandler
{
    ...
    protected int $thrownErrors = 0x1FFF; // E_ALL - E_DEPRECATED - E_USER_DEPRECATED
    protected int $scopedErrors = 0x1FFF; // E_ALL - E_DEPRECATED - E_USER_DEPRECATED
    protected int $tracedErrors = 0x77FB; // E_ALL - E_STRICT - E_PARSE
    protected int $screamedErrors = 0x55; // E_ERROR + E_CORE_ERROR + E_COMPILE_ERROR + E_PARSE
    protected int $loggedErrors = 0;
    protected Closure $configureException;
    ...
    protected ?BufferingLogger $bootstrappingLogger = null;

    protected static ?string $reservedMemory = null;
    protected static array $silencedErrorCache = [];
    protected static int $silencedErrorCount = 0;
    protected static int $exitCode = 0;

    /**
     * Registers the error handler.
     */
    public static function register(?ErrorHandler $handler = null, bool $replace = true): self
    {
        if (null === self::$reservedMemory) {
            self::$reservedMemory = str_repeat('x', 32768);
            register_shutdown_function(Closure::fromCallable([self::class, 'handleFatalError']));
        }
        ...
        $handler->isRoot = true;
        // Directly set this handler as the exception and error handler
        set_error_handler([$handler, 'handleError'], $handler->thrownErrors | $handler->loggedErrors);
        set_exception_handler([$handler, 'handleException']);
        $handler->setExceptionHandler([$handler, 'renderException']);


        // Exclude deprecation warnings and notices
        $nonDeprecationErrors = E_ALL & ~E_NOTICE & ~E_USER_NOTICE & ~E_STRICT & ~E_DEPRECATED & ~E_USER_DEPRECATED;

        $handler->throwAt($nonDeprecationErrors, true);
        ...
        // Adjust the error levels to exclude deprecation warnings
        //$nonDeprecationErrors = E_ALL & ~$handler->thrownErrors & ~E_DEPRECATED & ~E_USER_DEPRECATED;
        $nonDeprecationErrors = E_ALL & ~E_DEPRECATED & ~E_USER_DEPRECATED & ~E_NOTICE & ~E_STRICT;
     ...
     */
    protected function reRegister(int $prev): void
    {
    ...
    {
        return false;
        if (E_WARNING === $type && '"' === $message[0] && str_contains(
                $message,
        ...
        //echo "\n-------------handleError $message TYPE: " . $type . " currentLevel: $level silenced: $silenced Log: $log Throw: $throw";
        ...
        if (!$throw && !($type & $level)) {
            if (!isset(self::$silencedErrorCache[$id = $file . ':' . $line])) {
                $lightTrace = $this->tracedErrors & $type ? $this->cleanTrace(
                    debug_backtrace(
    ...
    public static function handleFatalError(?array $error = null): void
    {
        ...
        if (!$handler) {
            if (null === $error && $exitCode = self::$exitCode) {
                register_shutdown_function('register_shutdown_function', static function () use ($exitCode) {
        ...
        if ($error && $error['type'] &= E_PARSE | E_ERROR | E_CORE_ERROR | E_COMPILE_ERROR) {
            ...
            if (str_starts_with($error['message'], 'Allowed memory') || str_starts_with(
                    $error['message'],
                    'Out of memory'
                )) {
                $fatalError = new OutOfMemoryError(
    ...
    {
        $renderer = in_array(PHP_SAPI, ['cli', 'phpdbg', 'embed'], true) ? new CliErrorRenderer(
        ...
        echo esc_html($exception->getAsString());
    }
     ...
     */
    protected function cleanTrace(array $backtrace, int $type, string &$file, int &$line, bool $throw): array
    {
     ...
     */
    protected function parseAnonymousClass(string $message): string
    {
        return preg_replace_callback(
            '/[a-zA-Z_\x7f-\xff][\\\\a-zA-Z0-9_\x7f-\xff]*+@anonymous\x00.*?\.php(?:0x?|:[0-9]++\$)[0-9a-fA-F]++/',
            static fn($m) => class_exists($m[0], false) ? (get_parent_class($m[0]) ?: key(
